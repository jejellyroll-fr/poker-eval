...
/* Profiles the tree itself marks complete ("any hand", no combo list) drive
   the spot when no --range overrides that player: a tree generated by
   pe-preflop-tree carries its own ranges, and a full PLO5/PLO6 range cannot
   be spelled out.  Returns 1 when player p has such a profile on the tree's
   preflop root. */
static int tree_profile_is_complete(const mpf_tree_def_t *tree, int player)
{
    if (!tree || player < 0 || player >= MPF_MAX_PLAYERS)
        return 0;
    for (int i = 0; i < tree->range_profile_count; ++i)
    {
        const mpf_tree_range_profile_t *profile = &tree->range_profiles[i];
        if (profile->complete && profile->player == player)
            return 1;
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    /* The tree's own "complete" profiles fill in the ranges nobody named on
       the command line; an explicit --rangeN always wins. */
    for (int p = 0; p < num_players; ++p)
    {
        if (!range_text[p] && tree_profile_is_complete(tree, p))
            complete_range[p] = 1;
    }
    for (int p = 0; p < num_players; ++p)
    {
