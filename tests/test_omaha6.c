...
#include <poker_eval/games/rules_omaha6.h>
...
static int run_test_omaha6(void) {
    printf("Testing Omaha 6 (PLO6)...\n");

    /* Case 1: Simple PLO6 High check */
    /* Board: As Ks Qs Js 2d */
    /* Hand 1: Ts 9s 2c 3c 4c 5c (Straight Flush T-A) - uses Ts 9s */
    /* Hand 2: Ah Ad 2h 3h 4h 5h (Trip Aces) */

    StdDeck_CardMask board = parse_hand("As Ks Qs Js 2d");
    StdDeck_CardMask pockets[2];
    pockets[0] = parse_hand("Ts 9s 2c 3c 4c 5c");
    pockets[1] = parse_hand("Ah Ad 2h 3h 4h 5h");
    ...
    printf("PASS: Simple PLO6 exhaustive\n");
    return 0;
}

int main(void) {
    if (run_test_omaha6() != 0) return 1;
    return 0;
}
