/* test_lowball_mc_small.c - sanity check for Joker lowball Monte Carlo */
...
static void setup_pockets(StdDeck_CardMask pockets[2]) {
    ...
    /* Two simple distinct low-oriented holdings */
    ...
    StdDeck_CardMask_SET(pockets[1], StdDeck_MAKE_CARD(StdDeck_Rank_4, StdDeck_Suit_SPADES));
...
int main(void) {
    ...
    int iters = 200;
    int rv = enumSample(game_lowball, pockets, board, dead, 2, 0, iters, 0, &result);
    if (rv != 0) {
    ...
    /* Lowball is low-only, still expect sum EV ≈ iters */
    ...
    printf("Lowball Joker MC small test passed.\n");
    return 0;
}
