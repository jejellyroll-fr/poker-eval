...
| `test_best_response_exploitability` (ISSUE-12, #168) | Kuhn 2p, 3-player majority, perfect-information Gambit game | Engine `cfr_best_response_value`/`cfr_exploitability` (and multiway variants) equal an independent recursive oracle; deliberately exploitable policies (Always Fold → BR=+1, Always Call → BR=+0.5) match closed forms; multiway NashConv == independent oracle; exact Gambit-LP equilibrium → exploitability `e < 1e-7` |
