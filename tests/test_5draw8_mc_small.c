/* test_5draw8_mc_small.c - sanity check for 5-draw8 Monte Carlo (JokerDeck) */
...
int main(void) {
    ...
    int iters = 200;
    int rv = enumSample(game_5draw8, pockets, board, dead, 2, 0, iters, 0, &result);
    if (rv != 0) {
    ...
    /* Hi/Lo split: still one pot per iteration; sum EV should be close to iters */
    ...
    printf("5-draw8 MC small test passed.\n");
    return 0;
}
