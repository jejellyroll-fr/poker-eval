/* test_5draw8_exhaustive_multi.c - Validate exhaustive multi-hand enumeration for 5-Draw8 (JokerDeck) */
...
int main(void) {
    ...
    int rv = enumExhaustive_dispatch(game_5draw8, pockets, board, dead_std, 2, 0, 0, &result);
    if (rv != 0) {
    ...
    printf("5-draw8 exhaustive multi-hand: nsamples=%u expected=%llu\n", result.nsamples, expected);
    return 0;
}
