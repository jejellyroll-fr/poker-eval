...
JokerRules_HandVal_toString(HandVal handval, char *outString) {
  ...
  int written = snprintf(p, remaining, "%s (", JokerRules_handTypeNames[htype]);
  advance_cursor(&p, &remaining, written);

  if (remaining > 0 && JokerRules_nSigCards[htype] >= 1) {
    written = snprintf(p, remaining, "%c",
                       StdDeck_rankChars[HandVal_TOP_CARD(handval)]);
    advance_cursor(&p, &remaining, written);
  }

  if (remaining > 0 && JokerRules_nSigCards[htype] >= 2) {
    written = snprintf(p, remaining, " %c",
                       StdDeck_rankChars[HandVal_SECOND_CARD(handval)]);
    advance_cursor(&p, &remaining, written);
  }

  if (remaining > 0 && JokerRules_nSigCards[htype] >= 3) {
    written = snprintf(p, remaining, " %c",
                       StdDeck_rankChars[HandVal_THIRD_CARD(handval)]);
    advance_cursor(&p, &remaining, written);
  }

  if (remaining > 0 && JokerRules_nSigCards[htype] >= 4) {
    written = snprintf(p, remaining, " %c",
                       StdDeck_rankChars[HandVal_FOURTH_CARD(handval)]);
    advance_cursor(&p, &remaining, written);
  }

  if (remaining > 0 && JokerRules_nSigCards[htype] >= 5) {
    written = snprintf(p, remaining, " %c",
