...
#include <poker_eval/utils/mktable.h>
...
doNBitsTable(void) {
  ...
  for (i=0; i < StdDeck_N_RANKMASKS; i++) 
    MakeTable_outputUInt8((uint8)n_bits_func(i));
...
doTopCardTable(void) {
  ...
  for (i=0; i < StdDeck_N_RANKMASKS; i++) 
    MakeTable_outputUInt8((uint8)top_card_func(i));
...
doStraightTable(void) {
  ...
  for (i=0; i < StdDeck_N_RANKMASKS; i++) 
    MakeTable_outputUInt8((uint8)straight_func(i));
