/*
 * bench_cfr_omaha8_river.c - MCCFR on Omaha Hi/Lo 8-or-better HU river (fixed deal)
 */
...
static void dump_cb(uint64_t key,
                    int n,
                    const double *regret,
                    const double *avg,
                    double ev_sum,
...
static void dump_json_cb(uint64_t key,
                         int n,
                         const double *regret,
                         const double *avg,
                         double ev_sum,
...
static int read_o8_deal_line(const char *line, mask_t *h0, mask_t *h1, mask_t *board)
{
    int idx[13];
    int n = 0;
    const char *p = line;
...
int main(int argc, char **argv)
{
    ...
    if (!ctx)
    {
        fprintf(stderr, "Failed to initialize Omaha8 evaluation context\n");
        return 1;
    }
    int deals = 50;
    int iters = 1000;
    ...
    for (int d = 0; d < deals; ++d)
    {
        ...
        if (fds)
        {
            if (!fgets(line, sizeof(line), fds) || !read_o8_deal_line(line, &h0, &h1, &bd))
            {
        ...
        {
            /* Sample random distinct 13 cards */
            srand(seed + d * 101u);
            int used[52] = {0};
            int arr[13];
            int k = 0;
        ...
        cfr_game_t game;
        o8_state_t st;
        o8_build_game(ctx, h0, h1, bd, &game, &st);
        ...
        if (deals > 0)
        {
            ...
            if (eta < 0.0)
                eta = 0.0;
            fprintf(stderr, "[omah8] deal %d/%d  elapsed %.2fs  est %.2fs  eta %.2fs\n",
        ...
        if (d == 0)
        {
            ...
            if (strat_readable)
            {
                ...
                if (fr)
                {
                    ...
                    ctxd.ev_root = policy_ev_p0;
                    cfr_storage_iterate_stats(storage, dump_cb, &ctxd);
                    fclose(fr);
                }
            }
            if (strat_json)
            {
                ...
                if (fj)
                {
                    ...
                    dump_ctx_t ctxj = {fj, bucket_mode, 4, {1.0 / 3.0, 0.5, 0.75, 1.0}, 1, 0.0, 0.0};
                    ctxj.ev_root = policy_ev_p0;
                    cfr_storage_iterate_stats(storage, dump_json_cb, &ctxj);
                    fprintf(fj, "\n  ]\n}\n");
                    fclose(fj);
                }
            }
    ...
    if (deals > 0)
    {
        fprintf(stderr, "[omah8] completed in %.2fs\n", total_time);
        fflush(stderr);
    }
    printf("CFR Omaha8 HU river: deals=%d, iters=%d/deal, total_time=%.3f s\n", deals, iters, total_time);
