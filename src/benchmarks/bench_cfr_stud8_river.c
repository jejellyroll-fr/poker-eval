/*
 * bench_cfr_stud8_river.c - MCCFR on Seven-Card Stud Hi/Lo 8-or-better HU (final state)
 */
                    ...
                    void *u)
{
    ...
    const char *bname = (b_cls == 0 ? "-" : eval_hand_class_name((hand_class_t)b_cls));
...
static int read_s8_deal_line(const char *line, mask_t *s0, mask_t *s1)
{
    int idx[14];
    int n = 0;
    const char *p = line;
    ...
    *s1 = MASK_EMPTY;
    for (int i = 0; i < 7; i++)
        *s0 = mask_set(*s0, idx[i]);
    for (int i = 7; i < 14; i++)
        *s1 = mask_set(*s1, idx[i]);
    return 1;
}
...
int main(int argc, char **argv)
{
    ...
    for (int d = 0; d < deals; ++d)
    {
        ...
        if (fds)
        {
            if (!fgets(line, sizeof(line), fds) || !read_s8_deal_line(line, &s0, &s1))
            {
        ...
        else
        {
            ...
            for (int i = 0; i < 7; i++)
                s0 = mask_set(s0, arr[i]);
            for (int i = 7; i < 14; i++)
                s1 = mask_set(s1, arr[i]);
        }
        cfr_game_t game;
        s8_state_t st;
        s8_build_game(ctx, s0, s1, &game, &st);
    ...
    if (fcsv)
        fclose(fcsv);
    printf("CFR Stud8 HU (final): deals=%d, iters=%d/deal, total_time=%.3f s\n", deals, iters, total_time);
