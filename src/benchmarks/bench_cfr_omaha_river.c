/*
 * bench_cfr_omaha_river.c - MCCFR on Omaha (high-only) HU river (fixed deal)
 */
...
static void dump_cb(uint64_t key, int n, const double *regret, const double *avg, void *u)
{
    ...
    /* bucket_mode 4/5/6 pack an 8-bit learned strength/texture bucket id over
     * bits 48..55, so the p_cls / coarse split does not apply there. */
    unsigned bucket_id = (unsigned)((key >> 48) & 0xFFull);
    if (C->bucket_mode == 4 || C->bucket_mode == 5 || C->bucket_mode == 6)
    {
        coarse = bucket_id;
        p_cls = 0u;
    }
    ...
    fprintf(C->f, "%llu,%u,%s,%s,%u,0x%04X,%u,%u,%d,%s,%.6f,%.6f,%.6f,%.6f\n",
            (unsigned long long)key, p, bname ? bname : "-", pname ? pname : "-", (C->bucket_mode == 3 || C->bucket_mode == 4 || C->bucket_mode == 5 || C->bucket_mode == 6) ? coarse : 0u,
...
static int read_omaha_deal_line(const char *line, mask_t *h0, mask_t *h1, mask_t *board)
{
    int idx[13];
    int n = 0;
    const char *p = line;
...
if (!resume_path)
{
    ...
    for (int d = 0; d < deals; ++d)
    {
        ...
        if (fds)
        {
            if (!fgets(line, sizeof(line), fds) || !read_omaha_deal_line(line, &h0, &h1, &bd))
            {
        ...
        {
            /* Sample random distinct 13 cards for Omaha */
            srand(seed + d * 101u);
            int used[52] = {0};
            int arr[13];
            int k = 0;
            ...
            for (int i = 8; i < 13; i++)
                bd = mask_set(bd, arr[i]);
        }

        cfr_game_t game;
        omaha_river_state_t st;
        omaha_build_game(ctx, h0, h1, bd, &game, &st);
         ...
         * merging. A strength table is board-specific, trained on the
         * canonicalized board of this deal (deterministic, shared between deals
         * of the same board). The texture level is just stored on the state. */
        ...
        if (bucket_mode == 5 || bucket_mode == 7)
        {
            ...
            sopts.hole_cards = 4;
            sopts.seed = cluster_seed ? cluster_seed : seed;
            sopts.max_samples = cluster_samples;
    ...
    printf("CFR Omaha (high-only) HU river: deals=%d, iters=%d/deal, total_time=%.3f s\n", deals, iters, total_time);
