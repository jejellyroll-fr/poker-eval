/*
 * bench_cfr_shortdeck_river.c - MCCFR on Short Deck Hold'em HU river (fixed deal)
 */
...
static int read_shortdeck_deal_line(const char *line, mask_t *h0, mask_t *h1, mask_t *board)
{
    int idx[9];
    int n = 0;
    const char *p = line;
...
{
    EvalConfig cfg = eval_config_shortdeck();
...
if (checkpoint_interval == 0)
{
    ...
    for (int d = 0; d < deals; ++d)
    {
        ...
        if (fds)
        {
            if (!fgets(line, sizeof(line), fds) || !read_shortdeck_deal_line(line, &h0, &h1, &bd))
            {
        ...
        {
            /* Sample random distinct 9 cards for Short Deck (2 hole each + 5 board) */
            srand(seed + d * 101u);
            int used[36] = {0};
            int arr[9];
            int k = 0;
        ...
        cfr_game_t game;
        shortdeck_state_t st;
        shortdeck_build_game(ctx, h0, h1, bd, &game, &st);
    ...
    printf("CFR Short Deck Hold'em HU river: deals=%d, iters=%d/deal, total_time=%.3f s\n", deals, iters, total_time);
