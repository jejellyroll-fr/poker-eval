/*
 * bench_cfr_razz_river.c - MCCFR on Razz HU river (fixed deal)
 */
...
static int read_razz_deal_line(const char *line, mask_t *h0, mask_t *h1)
{
    int idx[14];
    int n = 0;
    const char *p = line;
...
if (checkpoint_interval == 0)
{
    ...
    for (int d = 0; d < deals; ++d)
    {
        ...
        if (fds)
        {
            if (!fgets(line, sizeof(line), fds) || !read_razz_deal_line(line, &h0, &h1))
            {
        ...
        {
            /* Sample random distinct 14 cards for Razz (7 each) */
            srand(seed + d * 101u);
            int used[52] = {0};
            int arr[14];
            int k = 0;
        ...
        cfr_game_t game;
        razz_state_t st;
        razz_build_game(ctx, h0, h1, &game, &st);
    ...
    printf("CFR Razz HU river: deals=%d, iters=%d/deal, total_time=%.3f s\n", deals, iters, total_time);
