...
parseArgs(int argc, char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
	...
	if (++i == argc) goto error;
      ...
      if (StdDeck_stringToCard(argv[i], &c) == 0)
        goto error;
      if (!StdDeck_CardMask_CARD_IS_SET(gPeggedCards, c)) {
        StdDeck_CardMask_SET(gPeggedCards, c);
        ++gNPegged;
      };
    }
  }
...
main(int argc, char **argv) {
  StdDeck_CardMask hand, deadCards, cards;
  ...
  parseArgs(argc, argv);
  StdDeck_CardMask_OR(deadCards, gDeadCards, gPeggedCards);
  ...
  if (gNPegged > 0) {
    char cardstr[100];
    StdDeck_maskString(gPeggedCards, cardstr);
