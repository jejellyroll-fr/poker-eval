...
int gNCommon, gNDead;
StdDeck_CardMask gDeadCards, gCommonCards, gPlayerCards[2];
  ...
  for (i = 1; i < argc; ++i) {
	...
	if (++i == argc) goto error;
        if (StdDeck_stringToCard(argv[i], &c) == 0)
          goto error;
        if (!StdDeck_CardMask_CARD_IS_SET(gDeadCards, c)) {
          ++gNDead;
...
{
  StdDeck_CardMask cards, p0, p1, c0, c1;
  HandVal h0, h1;
  int h0_count=0, h1_count=0, tie_count=0, count=0;

  StdDeck_CardMask_RESET(gDeadCards);
  StdDeck_CardMask_RESET(gCommonCards);
  StdDeck_CardMask_RESET(gPlayerCards[0]);
  StdDeck_CardMask_RESET(gPlayerCards[1]);
  parseArgs(argc, argv);

  StdDeck_CardMask_OR(p0, gPlayerCards[0], gCommonCards);
  StdDeck_CardMask_OR(p1, gPlayerCards[1], gCommonCards);
  StdDeck_CardMask_OR(gDeadCards, gDeadCards, gCommonCards);
  StdDeck_CardMask_OR(gDeadCards, gDeadCards, gPlayerCards[0]);
  StdDeck_CardMask_OR(gDeadCards, gDeadCards, gPlayerCards[1]);

  ENUMERATE_N_CARDS_D(cards, 5-gNCommon, gDeadCards, 
                      {
                        ++count;
                        StdDeck_CardMask_OR(c0, p0, cards);
                        h0 = Hand_EVAL_N(c0, 7);
                        StdDeck_CardMask_OR(c1, p1, cards);
                        h1 = Hand_EVAL_N(c1, 7);
  ...
  if (gNCommon > 0) {
    char cardstr[100];
    StdDeck_maskString(gCommonCards, cardstr);
    printf(" containing %s ", cardstr);
  }
  if (gNDead) {
    char cardstr[100];
    StdDeck_maskString(gDeadCards, cardstr);
    printf(" with %s removed ", cardstr);
  }
  printf("\n");
  ...
  {
    char cardstr[100];
    StdDeck_maskString(gPlayerCards[0], cardstr);
  ...
  {
    char cardstr[100];
    StdDeck_maskString(gPlayerCards[1], cardstr);
    printf("  %s  %7d %6.2f   %7d %6.2f   %7d %6.2f     %5.3f\n",
           cardstr,
           h1_count, 100.0*h1_count/count,
           h0_count, 100.0*h0_count/count,
