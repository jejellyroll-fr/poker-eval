...
static void format_mask_string(StdDeck_CardMask mask, char *buf, size_t buflen)
{
  static const char ranks[] = "23456789TJQKA";
  static const char suits[] = "cdhs";
  size_t used = 0;
  buf[0] = '\0';
  for (int card = StdDeck_N_CARDS - 1; card >= 0; --card) {
    if (!StdDeck_CardMask_CARD_IS_SET(mask, card))
      continue;
    if (used + 2 >= buflen)
      break;
    buf[used++] = ranks[StdDeck_RANK(card)];
    buf[used++] = suits[StdDeck_SUIT(card)];
  }
  buf[used] = '\0';
  if (used == 0 && buflen > 1)
    snprintf(buf, buflen, "-");
...
static int run_opencl_monte_carlo(
    enum_game_t game,
    UniversalCardMask pockets[],
    ...
    int terse)
{
  if (game != game_holdem)
    return 1;
  if (deckType != UNIVERSAL_DECK_STANDARD)
    return 1;
  if (npockets <= 0 || npockets > ENUM_MAXPLAYERS)
    return 1;
  ...
  int cards_to_draw = params->maxboard - nboard;
  if (cards_to_draw < 0 || cards_to_draw > 5)
    return 1;

  StdDeck_CardMask spockets[ENUM_MAXPLAYERS];
  for (int i = 0; i < npockets; ++i)
    spockets[i] = pockets[i].std;
  StdDeck_CardMask sboard = board.std;
  StdDeck_CardMask sdead = dead.std;

  StdDeck_CardMask used = sdead;
  for (int i = 0; i < npockets; ++i)
    StdDeck_CardMask_OR(used, used, spockets[i]);
  StdDeck_CardMask_OR(used, used, sboard);

  int available_cards[StdDeck_N_CARDS];
  int avail_count = 0;
  for (int card = 0; card < StdDeck_N_CARDS; ++card) {
    StdDeck_CardMask cmask = StdDeck_MASK(card);
    if (!StdDeck_CardMask_ANY_SET(used, cmask))
      available_cards[avail_count++] = card;
  }
  if (avail_count < cards_to_draw)
    return -1;
  ...
  gpu_eval_context_t *ctx = opencl_gpu_eval_init_game(0, max_batch, 1, config);
  if (!ctx) {
    fprintf(stderr, "OpenCL context init failed.\n");
    return -1;
  }

  size_t batch_size = (size_t)((iterations < (int)max_batch) ? iterations : (int)max_batch);
  StdDeck_CardMask *boards = malloc(batch_size * sizeof(StdDeck_CardMask));
  StdDeck_CardMask *hole_cards = malloc(batch_size * npockets * sizeof(StdDeck_CardMask));
  if (!boards || !hole_cards) {
    fprintf(stderr, "Allocation failure for OpenCL buffers.\n");
    free(boards);
    free(hole_cards);
  ...
  gpu_eval_result_t gpu_result;
  memset(&gpu_result, 0, sizeof(gpu_result));

  double equity[ENUM_MAXPLAYERS] = {0.0};
  double wins[ENUM_MAXPLAYERS] = {0.0};
  double ties[ENUM_MAXPLAYERS] = {0.0};
  double total_samples = 0.0;

  int samples_remaining = iterations;
  while (samples_remaining > 0) {
    int current_batch = (samples_remaining < (int)batch_size) ? samples_remaining : (int)batch_size;

    for (int b = 0; b < current_batch; ++b) {
      StdDeck_CardMask sample_board = sboard;
      int tmp_avail[StdDeck_N_CARDS];
      memcpy(tmp_avail, available_cards, avail_count * sizeof(int));
      int tmp_count = avail_count;
      for (int draw = 0; draw < cards_to_draw; ++draw) {
        int idx = rand() % tmp_count;
        int card = tmp_avail[idx];
        tmp_avail[idx] = tmp_avail[tmp_count - 1];
        tmp_count--;
        StdDeck_CardMask_OR(sample_board, sample_board, StdDeck_MASK(card));
      }
      boards[b] = sample_board;
      for (int p = 0; p < npockets; ++p)
        hole_cards[b * npockets + p] = spockets[p];
    }

    if (opencl_gpu_eval_batch_boards(ctx, boards, hole_cards, current_batch, npockets, &gpu_result) != 0) {
      fprintf(stderr, "OpenCL batch evaluation failed.\n");
      free(boards);
      free(hole_cards);
      if (gpu_result.hand_values)
    ...
    for (int b = 0; b < current_batch; ++b) {
      int base = b * npockets;
      HandVal best = gpu_result.hand_values[base];
      int winner_flags[ENUM_MAXPLAYERS] = {0};
      winner_flags[0] = 1;
      int winner_count = 1;
      for (int p = 1; p < npockets; ++p) {
        HandVal hv = gpu_result.hand_values[base + p];
        if (hv > best) {
          best = hv;
          memset(winner_flags, 0, sizeof(winner_flags));
          winner_flags[p] = 1;
          winner_count = 1;
        } else if (hv == best) {
          winner_flags[p] = 1;
  ...
  char labels[ENUM_MAXPLAYERS][32];
  for (int i = 0; i < npockets; ++i)
    format_mask_string(spockets[i], labels[i], sizeof(labels[i]));

  if (terse) {
    for (int i = 0; i < npockets; ++i) {
      double eq = equity[i] / total_samples;
      printf("%s=%.4f", labels[i], eq);
      if (i + 1 < npockets)
        printf(" ");
    }
    printf("\n");
  } else {
    printf("\n=== OpenCL Monte Carlo Hold'em ===\n");
    printf("Samples: %.0f\n", total_samples);
    printf("%-4s %-14s %10s %10s %10s\n", "#", "Input", "Equity", "Win%", "Tie%");
    for (int i = 0; i < npockets; ++i) {
      double eq = equity[i] / total_samples;
      double winpct = (wins[i] * 100.0) / total_samples;
      double tiepct = (ties[i] * 100.0) / total_samples;
      printf("%-4d %-14s %10.4f %10.2f %10.2f\n", i + 1, labels[i], eq, winpct, tiepct);
    }
    printf("\n");
  }
...
main(int argc, char **argv) {
  ...
  do {
    ...
    if (parseArgs(argc, argv, &game, &enumType, &niter,
      ...
      int handled = run_opencl_monte_carlo(game, pockets, board, dead, deckType,
                                           npockets, nboard, niter, terse);
      if (handled == 0) {
