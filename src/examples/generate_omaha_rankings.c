/*
 * generate_omaha_rankings.c - Generate a static Omaha hand rankings lookup
 *
 * Computes preflop equity (vs. a random 4-card hand) for every canonical
 * Omaha starting hand and writes a compact C header containing the canonical
 * keys sorted best-first. The Advanced Range Parser consumes this table to
 * expand percentage ranges (e.g. "15%", "20.5%") without running Monte-Carlo
 * at call time.
 *
 * The output header is committed to the tree (src/range/omaha_hand_rankings.h);
 * re-run this tool only when the definition of "best" hand needs refreshing.
 *
 * Usage: generate_omaha_rankings <output.h> [samples_per_hand] [seed]
 *
...
/* Total number of 4-card combinations from a 52-card deck: C(52,4) */
#define TOTAL_HANDS 270725u

typedef struct {
    omaha_hand_key_t key;
    float equity;
} ranked_entry_t;
...
/* Preflop equity of `hand` vs a random 4-card opponent, via Monte-Carlo.
 * Both players see the same 5-card board; the opponent pocket and the board
 * are dealt from the 48-card stub left over after removing `hand`. */
static double omaha_equity_vs_random(StdDeck_CardMask hand, int samples) {
    ...
    for (int it = 0; it < samples; ++it) {
        ...
        deal_random(&dead, &opp, 4);
        deal_random(&dead, &board, 5);

        pockets[1] = opp;
        enum_result_t res;
        enumSample(game_omaha, pockets, board, empty, 2, 5, 1, 0, &res);
        equity += res.ev[0];
    }
...
static int cmp_ranked_desc(const void *a, const void *b) {
    ...
    if (ea->equity < eb->equity) return 1;
    /* Tie-break by key so the ordering is fully deterministic */
...
int main(int argc, char *argv[]) {
    ...
    /* Enumerate every canonical 4-card hand exactly once.  A dynamic table
     * sized to the max canonical key space is too large without the whole
     * (uncommitted) 16MB equity table, so we enumerate all concrete hands and
     * keep the first representative for every distinct canonical key. */
    static omaha_hand_key_t avail[TOTAL_HANDS];    /* keys, one per canonical class */
    static uint8_t density[(1u << 22) / 8];        /* bit-set of visited keys */
        ...
        for (int j = i + 1; j < 52; ++j) {
            for (int k = j + 1; k < 52; ++k) {
                for (int l = k + 1; l < 52; ++l) {
                    ...
                    if (key < (1u << 22) && !(density[key >> 3] & (1u << (key & 7)))) {
    ...
    for (size_t idx = 0; idx < class_count; ++idx) {
        omaha_hand_key_t key = avail[idx];

        /* Reconstruct a representative hand so equity can be evaluated. */
        uint32_t rp = key >> 6;
        int ranks4[4] = { (int)((rp >> 12) & 0xF), (int)((rp >> 8) & 0xF),
                          (int)((rp >> 4) & 0xF), (int)(rp & 0xF) };
        ...
        for (int a4 = 0; a4 < 4 && !have_rep; ++a4) {
            int suits[4] = { a1, a2, a3, a4 };
            int valid = 1;
            for (int c = 0; c < 4 && valid; ++c)
                for (int d = c + 1; d < 4; ++d)
                    if (ranks4[c] == ranks4[d] && suits[c] == suits[d]) { valid = 0; break; }
            if (!valid) continue;

            StdDeck_CardMask m;
            StdDeck_CardMask_RESET(m);
            for (int c = 0; c < 4; ++c)
                StdDeck_CardMask_SET(m, StdDeck_MAKE_CARD(ranks4[c], suits[c]));
        ...
        StdDeck_CardMask_RESET(empty);
        ranks[idx].key = key;
        ranks[idx].equity = (float)omaha_equity_vs_random(rep, samples);
    ...
    fprintf(f, "/* Regenerate with: src/examples/generate_omaha_rankings          */\n");
    fprintf(f, "/* Canonical Omaha hands ranked by preflop equity vs random.      */\n");
    fprintf(f, "/* Canonical : suits normalized, ranks descending (Ace..2).       */\n\n");

    fprintf(f, "#ifndef POKER_EVAL_OMAHA_HAND_RANKINGS_H\n");
    fprintf(f, "#define POKER_EVAL_OMAHA_HAND_RANKINGS_H\n\n");
    fprintf(f, "#include \"poker_eval/equity/omaha_preflop.h\"\n\n");

    fprintf(f, "/* %zu canonical classes sorted by equity, best-first */\n", class_count);
    fprintf(f, "static const omaha_hand_key_t omaha_hand_rankings[] = {\n");
    for (size_t idx = 0; idx < class_count; ++idx) {
        fprintf(f, "    %uU,\n", (unsigned)ranks[idx].key);
    }
    fprintf(f, "};\n\n");
    fprintf(f, "#define OMAHA_HAND_RANKINGS_COUNT %zu\n", class_count);
    fprintf(f, "#define OMAHA_TOTAL_HANDS     %u\n", TOTAL_HANDS);
    fprintf(f, "\n#endif /* POKER_EVAL_OMAHA_HAND_RANKINGS_H */\n");
