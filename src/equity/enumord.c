...
enum_rankelem_compare(const void *p1, const void *p2) {
  const enum_rankelem_t *e1 = (const enum_rankelem_t *) p1;
  const enum_rankelem_t *e2 = (const enum_rankelem_t *) p2;
                   ...
                   int reverse)
{
  ...
  // Replace qsort with insertion sort for small nplayers to reduce overhead
  for (int i = 1; i < nplayers; ++i) {
    enum_rankelem_t key = elems[i];
    int j = i - 1;
    while (j >= 0 && elems[j].handval > key.handval) {
      elems[j+1] = elems[j];
      j--;
    }
    elems[j+1] = key;
