/* shortdeck_river_adapter.c - Short Deck Hold'em HU river */
#include <poker_eval/engine/solvers/cfr/shortdeck_river_adapter.h>
#include <string.h>
#include <stdlib.h>
#include <poker_eval/core/eval_context.h>

static bool sd_is_terminal(const void *s, double util[2]);
static int sd_num_actions(const void *s);
static bool sd_apply_action(const void *s, int a, void *out);
static int sd_player_to_act(const void *s);

static int sd_is_terminal_wrapper(cfr_game_t* game, uint64_t state_key, void* user_data) {
    double util[2];
    return sd_is_terminal((void*)state_key, util);
}

static double sd_get_utility_wrapper(cfr_game_t* game, uint64_t state_key, int player, void* user_data) {
    double util[2];
    sd_is_terminal((void*)state_key, util);
    return util[player];
}

static int sd_get_actions_wrapper(cfr_game_t* game, uint64_t state_key, int* out_actions, int max_actions, void* user_data) {
    int n = sd_num_actions((void*)state_key);
...
static uint64_t sd_apply_action_wrapper(cfr_game_t* game, uint64_t state_key, int action, void* user_data) {
    shortdeck_state_t* next_state = malloc(sizeof(shortdeck_state_t));
    sd_apply_action((void*)state_key, action, next_state);
...
static bool sd_is_terminal(const void *s, double util[2])
{
    const shortdeck_state_t *st = (const shortdeck_state_t *)s;
    if (st->hist & (1u << 5))
    {
    ...
    {
        eval_t v0 = pe_eval_7c(st->ctx, st->h0 | st->board);
        eval_t v1 = pe_eval_7c(st->ctx, st->h1 | st->board);
        if (v0 > v1)
        {
            util[0] = st->pot;
            util[1] = -st->pot;
...
static int sd_player_to_act(const void *s) { return ((const shortdeck_state_t *)s)->to_act; }
static int sd_num_actions(const void *s)
{
    const shortdeck_state_t *st = (const shortdeck_state_t *)s;
...
{
    const shortdeck_state_t *st = (const shortdeck_state_t *)s;
    shortdeck_state_t *ns = (shortdeck_state_t *)out;
    *ns = *st;
    if (ns->to_call > 0.0)
    {
        if (a == 0)
...
static uint64_t sd_infoset_key(const void *s)
{
    const shortdeck_state_t *st = (const shortdeck_state_t *)s;
    int p = st->to_act;
...
void shortdeck_build_game(const EvalContext *ctx, mask_t h0, mask_t h1, mask_t board, cfr_game_t *out_game, shortdeck_state_t *out_state)
{
    ...
    out_game->is_terminal = sd_is_terminal_wrapper;
    ...
    out_game->get_infoset_key = sd_infoset_key;
