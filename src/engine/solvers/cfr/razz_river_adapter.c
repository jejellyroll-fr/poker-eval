/* razz_river_adapter.c - Razz (A-5 lowball) HU river - using high eval inverted for simplicity */
...
static bool razz_is_terminal(const void *s, double util[2]);
static int razz_num_actions(const void *s);
static bool razz_apply_action(const void *s, int a, void *out);
static int razz_player_to_act(const void *s);

static int razz_is_terminal_wrapper(cfr_game_t* game, uint64_t state_key, void* user_data) {
    double util[2];
    return razz_is_terminal((void*)state_key, util);
}

static double razz_get_utility_wrapper(cfr_game_t* game, uint64_t state_key, int player, void* user_data) {
    double util[2];
    razz_is_terminal((void*)state_key, util);
    return util[player];
}

static int razz_get_actions_wrapper(cfr_game_t* game, uint64_t state_key, int* out_actions, int max_actions, void* user_data) {
    int n = razz_num_actions((void*)state_key);
...
static bool razz_is_terminal(const void *s, double util[2])
{
    const razz_state_t *st = (const razz_state_t *)s;
    if (st->hist & (1u << 5))
    {
    ...
    {
        /* For Razz: compare best A-5 lows from 7 cards */
        eval_t hi_tmp = 0;
        eval_t lo_eval0 = 0, lo_eval1 = 0;
        bool loq0 = false, loq1 = false;
        if (!pe_eval_7c_hilo(st->ctx, st->seven0, &hi_tmp, &lo_eval0, &loq0))
            loq0 = false;
        if (!pe_eval_7c_hilo(st->ctx, st->seven1, &hi_tmp, &lo_eval1, &loq1))
            loq1 = false;
        LowHandVal lo0 = loq0 ? (LowHandVal)lo_eval0 : LowHandVal_NOTHING;
        LowHandVal lo1 = loq1 ? (LowHandVal)lo_eval1 : LowHandVal_NOTHING;
        if (lo0 < lo1)
        {
            util[0] = st->pot;
            util[1] = -st->pot;
...
static int razz_player_to_act(const void *s) { return ((const razz_state_t *)s)->to_act; }
static int razz_num_actions(const void *s)
{
    const razz_state_t *st = (const razz_state_t *)s;
...
{
    const razz_state_t *st = (const razz_state_t *)s;
    razz_state_t *ns = (razz_state_t *)out;
    *ns = *st;
    if (ns->to_call > 0.0)
    {
        if (a == 0)
...
static uint64_t razz_infoset_key(const void *s)
{
    const razz_state_t *st = (const razz_state_t *)s;
    int p = st->to_act;
...
void razz_build_game(const EvalContext *ctx, mask_t seven0, mask_t seven1, cfr_game_t *out_game, razz_state_t *out_state)
{
    ...
    out_game->is_terminal = razz_is_terminal_wrapper;
