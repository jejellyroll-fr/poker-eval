...
### ✅ Phase 3 - Full Hand Rankings (Done)

- ✅ **Complete Omaha hand rankings** — percentages now resolve through a
  committed ranked table of all 20,410 canonical starting-hand classes
  (`src/range/omaha_hand_rankings.h`, generated by
  `src/examples/generate_omaha_rankings`), so `N%` expands to the actual top
  N% of all 270,725 starting hands
- ✅ **Deterministic expansion** — best-first walk over the ranked table with
  reverse canonical-key → concrete-hand expansion
- ✅ **Dead-card aware** — hands using dead cards are excluded
- ✅ **Caching** of frequent percentage patterns (`ARP_CACHE_SIZE`)
...
1. **Advanced patterns**: Some complex PLO patterns not supported
2. **Performance**: Not optimized for very large ranges
...
### Phase 3 — done
- [x] Percentage improvement with full hand rankings
- [x] Intelligent pattern caching
- [x] Dead-card aware percentage expansion
...
the Phase 2 operators plus Phase 3 full hand rankings; see issue #33.