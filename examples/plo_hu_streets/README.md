...
Trees generated by `pe-preflop-tree` are mpf_tree documents now: resolve them
directly with `mpf_run_with_metrics --tree FILE --rules <game>`, with or
without `--rangeN` (a profile marked `"complete":true` in the tree draws that
player's holes from the live deck when no `--range` names them).  The tool
also roots its trees at flop, turn or river with `--street` + `--board`
(starting from `--pot`, no blind posting), so it can regenerate per-street
trees like the ones in this directory — e.g. the PLO5 flop file matches
`pe-preflop-tree --players 2 --stack 96.5 --stack 96.5 --street flop
--board Ks9d4c --pot 7 --raises 7 --game plo5`.  These hand-written files
carry no range profiles, so ranges come from `--rangeN` or the `100%`
default.
