    ...
    for(suit=0; suit<4; suit++) {
      if(nBitsTable[suits[suit]] >= 5) {
        if(straightTable[suits[suit]]) {
          retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_STFLUSH) + HandVal_TOP_CARD_VALUE(straightTable[suits[suit]]);
          return retval;
        } else {
          retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_FLUSH) + topFiveCardsTable[suits[suit]];
          return retval;
        }
      }
    } 
