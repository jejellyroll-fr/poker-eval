...
#define StdDeck_RANK_MASK ((uint32)((1u << StdDeck_Rank_COUNT) - 1u))

static inline uint32 StdDeck_SanitizeRankMask(uint32 ranks) {
  return ranks & StdDeck_RANK_MASK;
}
...
#define StdDeck_CardMask_SET_SPADES(cm, ranks)                                 \
  ((cm).cards.spades = StdDeck_SanitizeRankMask(ranks))
#define StdDeck_CardMask_SET_CLUBS(cm, ranks)                                  \
  ((cm).cards.clubs = StdDeck_SanitizeRankMask(ranks))
#define StdDeck_CardMask_SET_DIAMONDS(cm, ranks)                               \
  ((cm).cards.diamonds = StdDeck_SanitizeRankMask(ranks))
#define StdDeck_CardMask_SET_HEARTS(cm, ranks)                                 \
  ((cm).cards.hearts = StdDeck_SanitizeRankMask(ranks))
...
#define StdDeck_CardMask_RESET(mask)                                           \
  do {                                                                         \
    (mask).cards_nn.n1 = (mask).cards_nn.n2 = 0;                               \
  } while (0)
#endif

#ifdef USE_INT64
#define StdDeck_CardMask_IS_EMPTY(mask) ((mask).cards_n == 0)
#else
...
#else
#define StdDeck_CardMask_EQUAL(mask1, mask2)                                   \
  ((mask1).cards_nn.n1 == (mask2).cards_nn.n1 &&                               \
   (mask1).cards_nn.n2 == (mask2).cards_nn.n2)
#endif
...
/* Standard rules constants that depend on deck definitions */
#define StdRules_FIVE_STRAIGHT                                                 \
  ((1 << StdDeck_Rank_ACE) | (1 << StdDeck_Rank_2) | (1 << StdDeck_Rank_3) |   \
   (1 << StdDeck_Rank_4) | (1 << StdDeck_Rank_5))
...
#define StdDeck_maskToString(m, s)                                             \
  GenericDeck_maskToString(&StdDeck, ((void *)&(m)), (s))
