...
static inline void JokerDeck_copy_suit_to_std(StdDeck_CardMask *std_mask, uint32 ranks, int suit)
{
  uint32 sanitized_ranks = StdDeck_SanitizeRankMask(ranks);
  for (int rank = 0; rank < StdDeck_Rank_COUNT; rank++)
  {
    if (sanitized_ranks & (1u << rank))
    {
      StdDeck_CardMask_SET(*std_mask, StdDeck_MAKE_CARD(rank, suit));
    }
  }
}
...
do { \
  ...
  JokerDeck_copy_suit_to_std(&(sCards), JokerDeck_CardMask_SPADES(jCards), StdDeck_Suit_SPADES); \
  JokerDeck_copy_suit_to_std(&(sCards), JokerDeck_CardMask_HEARTS(jCards), StdDeck_Suit_HEARTS); \
  JokerDeck_copy_suit_to_std(&(sCards), JokerDeck_CardMask_CLUBS(jCards), StdDeck_Suit_CLUBS); \
  JokerDeck_copy_suit_to_std(&(sCards), JokerDeck_CardMask_DIAMONDS(jCards), StdDeck_Suit_DIAMONDS); \
} while (0)
