...
typedef struct {
  ...
  int		haslopot;
  int		hashipot;
  low_qualifier_t low_qualifier;  /* LOW_QUALIFIER_NONE, LOW_QUALIFIER_8, or LOW_QUALIFIER_7 */
  const char *	name;
} enum_gameparams_t;
...
extern POKEREVAL_EXPORT int enumExhaustive_dispatch(enum_game_t game, StdDeck_CardMask pockets[],
                                  ...
                                  enum_result_t *result);
typedef enum {
  enum_dispatch_backend_classic = 0,
  enum_dispatch_backend_eedc,
  enum_dispatch_backend_eedc_omaha_opt
} enum_dispatch_backend_t;

extern POKEREVAL_EXPORT enum_dispatch_backend_t enum_dispatch_last_backend(void);
extern POKEREVAL_EXPORT const char *enum_dispatch_backend_name(enum_dispatch_backend_t backend);
