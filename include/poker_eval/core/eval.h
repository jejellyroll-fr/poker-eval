...
StdDeck_StdRules_EVAL_N(StdDeck_CardMask cards, int n_cards)
{
  ...
  */
  if (n_ranks >= 5)
  {
    ...
    {
      if (straightTable[SS])
        return HandVal_HANDTYPE_VALUE(StdRules_HandType_STFLUSH) + HandVal_TOP_CARD_VALUE(straightTable[SS]);
      else
        retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_FLUSH) + topFiveCardsTable[SS];
    }
    else if (nBitsTable[SC] >= 5)
    {
      if (straightTable[SC])
        return HandVal_HANDTYPE_VALUE(StdRules_HandType_STFLUSH) + HandVal_TOP_CARD_VALUE(straightTable[SC]);
      else
        retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_FLUSH) + topFiveCardsTable[SC];
    }
    else if (nBitsTable[SD] >= 5)
    {
      if (straightTable[SD])
        return HandVal_HANDTYPE_VALUE(StdRules_HandType_STFLUSH) + HandVal_TOP_CARD_VALUE(straightTable[SD]);
      else
        retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_FLUSH) + topFiveCardsTable[SD];
    }
    else if (nBitsTable[SH] >= 5)
    {
      if (straightTable[SH])
        return HandVal_HANDTYPE_VALUE(StdRules_HandType_STFLUSH) + HandVal_TOP_CARD_VALUE(straightTable[SH]);
      else
        retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_FLUSH) + topFiveCardsTable[SH];
    }
    else
    {
      int st;
      ...
      if (st)
        retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_STRAIGHT) + HandVal_TOP_CARD_VALUE(st);
    };
  ...
  case 0:
    /* It's a no-pair hand */
    return HandVal_HANDTYPE_VALUE(StdRules_HandType_NOPAIR) + topFiveCardsTable[ranks];
    break;
  ...
  case 1:
  {
    ...
    retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_ONEPAIR) + HandVal_TOP_CARD_VALUE(topCardTable[two_mask]);
    ...
    kickers = (topFiveCardsTable[t] >> HandVal_CARD_WIDTH) & ~HandVal_FIFTH_CARD_MASK;
    retval += kickers;
  ...
  case 2:
    ...
    if (two_mask)
    {
      ...
      retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_TWOPAIR) + (topFiveCardsTable[two_mask] & (HandVal_TOP_CARD_MASK | HandVal_SECOND_CARD_MASK)) + HandVal_THIRD_CARD_VALUE(topCardTable[t]);
    ...
    else
    {
      ...
      retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_TRIPS) + HandVal_TOP_CARD_VALUE(topCardTable[three_mask]);
  ...
  default:
    ...
    if (four_mask)
    {
      ...
      retval = HandVal_HANDTYPE_VALUE(StdRules_HandType_QUADS) + HandVal_TOP_CARD_VALUE(tc) + HandVal_SECOND_CARD_VALUE(topCardTable[ranks ^ (1 << tc)]);
      return retval;
    };
    ...
    else
    {
      ...
      retval += HandVal_SECOND_CARD_VALUE(second);
      retval += HandVal_THIRD_CARD_VALUE(topCardTable[ranks ^ (1 << top) ^ (1 << second)]);
      return retval;
    };
  ...
  /* Should never happen */
  assert(0 && "Logic error in StdDeck_StdRules_EVAL_N");
}
