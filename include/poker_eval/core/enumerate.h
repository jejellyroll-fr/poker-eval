...
#define JOKERDECK_ENUMERATE_COMBINATIONS_D(set_var, num_sets, set_sizes, \
                                         dead_cards, action) \
do { \
  Combinations *_combos; \
  int *_ncombo; \
  int *_curIndex; \
  int **_curElem; \
  JokerDeck_CardMask *_curHand; \
  JokerDeck_CardMask _avail; \
  JokerDeck_CardMask _unavail; \
  int _lowestRoll; \
  int _couldIncr; \
  int _i, _j, _k; \
  int _debug_iter = 0; \
   \
  if (getenv("POKER_DEBUG") && getenv("POKER_DEBUG")[0] == '1') { \
    fprintf(stderr, "JOKERDECK_ENUMERATE_COMBINATIONS_D: Starting with num_sets=%d\n", num_sets); \
    for (_i=0; _i<num_sets; _i++) { \
      fprintf(stderr, "  Set %d: need %d cards\n", _i, set_sizes[_i]); \
    } \
    fprintf(stderr, "  Dead cards count: %d\n", JokerDeck_numCards(dead_cards)); \
  } \
   \
  _combos = (Combinations *) malloc(num_sets * sizeof(Combinations)); \
  _ncombo = (int *) malloc(num_sets * sizeof(int)); \
  _curIndex = (int *) malloc(num_sets * sizeof(int)); \
  _curElem = (int **) malloc(num_sets * sizeof(intptr_t)); \
  _curHand = (JokerDeck_CardMask *) malloc(num_sets * sizeof(JokerDeck_CardMask)); \
  for (_i=0; _i<num_sets; _i++) { \
    _combos[_i] = init_combinations(JokerDeck_N_CARDS, set_sizes[_i]); \
    _ncombo[_i] = num_combinations(_combos[_i]); \
    _curElem[_i] = (int *) malloc(set_sizes[_i] * sizeof(int)); \
    if (getenv("POKER_DEBUG")) { \
      fprintf(stderr, "  Set %d: %d possible combinations\n", _i, _ncombo[_i]); \
    } \
  } \
  _unavail = dead_cards; \
  _lowestRoll = 0; \
  do { \
    _debug_iter++; \
    if (getenv("POKER_DEBUG") && (_debug_iter % 1000 == 1 || _debug_iter <= 10)) { \
      fprintf(stderr, "  Iteration %d, lowestRoll=%d\n", _debug_iter, _lowestRoll); \
    } \
    /* for each player rolling over, find the lowest available hand */ \
    for (_i=_lowestRoll; _i<num_sets; _i++) { \
      /* for each candidate hand for player i */ \
      for (_j=0; _j<_ncombo[_i]; _j++) { \
        get_combination(_combos[_i], _j, _curElem[_i]); \
        JokerDeck_CardMask_RESET(set_var[_i]); \
        for (_k=0; _k<set_sizes[_i]; _k++) { \
          JokerDeck_CardMask_SET(set_var[_i], _curElem[_i][_k]); \
          if (getenv("POKER_DEBUG") && _debug_iter <= 5 && _j == 0) { \
            fprintf(stderr, "    Set %d, card %d: index=%d\n", _i, _k, _curElem[_i][_k]); \
          } \
        } \
        if (!JokerDeck_CardMask_ANY_SET(_unavail, set_var[_i])) \
          break;	/* this hand is available for player i */ \
      } \
      if (_j == _ncombo[_i]) { \
        fprintf(stderr, "ERROR: not enough cards for set %d\n", _i); \
        return(1); \
      } \
      JokerDeck_CardMask_OR(_unavail, _unavail, set_var[_i]); \
      _curIndex[_i] = _j; \
      if (getenv("POKER_DEBUG") && _debug_iter <= 5) { \
        fprintf(stderr, "  Set %d: using combination %d\n", _i, _j); \
      } \
    } \
 \
    { action } \
 \
    ...
    _couldIncr = 0; \
    for (_i=num_sets-1; _i>=0; _i--) { \
      JokerDeck_CardMask_NOT(_avail, set_var[_i]); \
      JokerDeck_CardMask_AND(_unavail, _unavail, _avail); \
      for (_j=_curIndex[_i]+1; _j<_ncombo[_i]; _j++) { \
        get_combination(_combos[_i], _j, _curElem[_i]); \
        JokerDeck_CardMask_RESET(set_var[_i]); \
        for (_k=0; _k<set_sizes[_i]; _k++) \
...
#define JOKERDECK_MONTECARLO_PERMUTATIONS_D(set_var, num_sets, set_sizes, \
                                           dead_cards, num_iter, action)   \
do {                                                                       \
  int _i, _j, _k, _c;                                                      \
                                                                           \
  for (_i=0; _i<num_iter; _i++) {                                          \
    JokerDeck_CardMask _used;                                              \
    _used = dead_cards;                                                    \
