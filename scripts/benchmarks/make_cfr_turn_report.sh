...
BIN="${BUILD_DIR}/bin/bench_cfr_holdem_turn"
OUT_DIR="${ROOT_DIR}/docs/reports"
DEALSET="${OUT_DIR}/cfr_turn_dealset.txt"
CSV="${OUT_DIR}/cfr_turn_report.csv"
...
if [ ! -x "${BIN}" ]; then
  ...
  cmake --build "${BUILD_DIR}" -j 8 --target bench_cfr_holdem_turn
fi

python3 "${ROOT_DIR}/scripts/generate_turn_dealset.py" 100 "${DEALSET}" 777777
...
# Stage-aware defaults if not provided (turn)
if [ -z "${BSIZES}" ]; then BSIZES="33,66,100"; fi
